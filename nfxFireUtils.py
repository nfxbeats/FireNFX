...
import utils
from nfxFirePadDefs import _initialSnap as snapInit
...
def RefreshChannelPads(fire):
    ...
    for padNum in ChannelPads:
        ...
        trkIdx = ChannelPads.index(padNum)
...
def GetScaleGrid(rootNote=0, startOctave=2, scale=HARMONICSCALE_MINORPENTATONIC):
    ...
    for colOffset in range(0, 8):
        for row in range(0, 4):
            ...
            _PadMaps[padIdx].IsRootNote = (
                colOffset == 0) or (colOffset == notesInScale)
...
def HandleFPCPress(fire, event, m):
    ...
    if(_showFPCColors):
        ...
        padNoteVal = _PAD_Notes[fpcNoteIdx] + \
...
def HandleChannelPads(fire, PadIndex):
    ...
    chanPadIdx = ChannelPads.index(PadIndex)
...
def InitAll(fire):
    ...
    GetScaleGrid(0, 3, HARMONICSCALE_MINORPENTATONIC)
